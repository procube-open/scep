...
1. GUI MSI の 1 ページ目、または silent install 用に別配布する `device-id-probe` が、対象 Windows マシンの `EK public` から canonical `device_id` を導出して表示する
2. 管理者がサーバー側で高 entropy の opaque `client_uid` を払い出し、`managed_client_type=windows-msi` と canonical `device_id` を preregister する
3. 管理者は preregistration 完了後にだけ初回発行用 `enrollment_secret` を発行し、サーバー側 client state を `INACTIVE -> ISSUABLE` に遷移させる
4. GUI MSI は `SERVER_URL` と `CLIENT_UID` を受け取り、read-only prereg-check endpoint で `device_id` 一致と `ISSUABLE` 到達を確認する。`not_issuable_yet` の間は同じ画面で `Retry` できる
5. silent install は別配布の `device-id-probe` で採取した canonical `device_id` を前提にしつつ、bootstrap 時に同じ prereg-check を fail-fast で実施する
6. MSI は `ENROLLMENT_SECRET` を受け取り、`expected_device_id` を含む設定と Windows Service を配置し、サービスを登録する
7. Windows Service は起動時・更新時に毎回 `EK public` から canonical `device_id` を再導出し、`expected_device_id` と一致しなければ blocked / mismatch state へ入って enrollment / renewal を止める
8. `windows-msi` client の nonce issuance では、サービスが `ek_public_b64` を添えて nonce を要求し、サーバーはそこから canonical `device_id` を再計算して registered / request / TPM material の一致を確認したときだけ nonce を返す
9. 初回発行ではサービスは one-time `enrollment_secret` を付けて SCEP の `PKIOperation` POST を行い、サーバーは `ISSUABLE` state の challenge 検証、credential activation を含む TPM 検証、CSR 鍵 binding を実施する
10. 更新では `ISSUED` state のまま既存証明書 + TPM attestation で自動更新し、`windows-msi` client では `UPDATABLE` / `PENDING` の legacy update-secret flow を使わない
11. 証明書 expiry 後や TPM / vTPM 交換時は、管理者が明示的に state と `device_id` を更新して再 enrollment させる

## Installer UX

前提:

- GUI MSI を primary flow とし、1 ページ目で canonical `device_id` を表示する
- silent install は別配布の `device-id-probe` を prerequisite にする
- `client_uid` は高 entropy の opaque ID を使い、人間向け別名は別管理にする
- 初回 `enrollment_secret` は preregistration 完了後にだけ発行する
- release 向け MSI / service / server では `DEVICE_ID_OVERRIDE` を禁止し、test harness / debug build にだけ残す

### Required Inputs

- `SERVER_URL`: SCEP サーバー URL
- `CLIENT_UID`: preregister 済みの opaque client identifier
- `ENROLLMENT_SECRET`: preregistration 完了後に発行された初回発行用 secret
...
### Test / Debug Only Inputs

- `DEVICE_ID_OVERRIDE`: release では禁止し、test harness / debug build にだけ残す

### UI Policy

- GUI の 1 ページ目では `EK public` から canonical `device_id` を計算して表示し、コピーしやすい導線を持たせる
- 次の画面で `SERVER_URL` と `CLIENT_UID` を受け取り、unauthenticated だが rate-limited な read-only prereg-check endpoint へ問い合わせる
- prereg-check は `client_not_found` / `device_id_mismatch` / `not_issuable_yet` のような限定的 reason code だけを返し、登録済み値自体は返さない
- `not_issuable_yet` の場合は同じ画面で `Retry` できるようにし、ready になるまで先へ進ませない
- prereg-check 成功後にだけ `ENROLLMENT_SECRET` 入力と install 実行を許可する
- サイレントインストールでも local `EK public` から canonical `device_id` を再導出し、同じ prereg-check を通せなければ bootstrap を失敗させる
- `CLIENT_UID` は opaque ID のため、基本は copy/paste 前提とし、人間向け alias は installer の authority にしない
...
Windows MSI 管理対象の `device_id` は、通常運用では次の単一ルールで決める。

1. 対象 Windows マシンの GUI MSI 1 ページ目または `device-id-probe` が、TPM の `EK public` を `SubjectPublicKeyInfo` DER に正規化する
2. その DER 全体に対して `SHA-256` を計算する
3. 得られた値を lowercase 64 文字の 16 進文字列に変換し、canonical `device_id` とする
4. 管理者はその canonical `device_id` を server へ事前登録し、Windows Service / helper は発行時にも同じ規則で再導出した値を送る

`DEVICE_ID_OVERRIDE` は release では禁止し、test harness / debug build に限って残す。

理由:

- 人手入力の `device_id` は誤入力に弱い
- TPM に紐づく識別子のほうが、なりすまし耐性が高い
- `EK public` は TPM の身元側 material として安定しており、temporary `AIK` / `AK` より device identity source に向く
- current helper はすでに `ek_public_b64` を canonical attestation に載せているため、`device_id` 導出に既存 material を再利用できる

運用方針:

- GUI MSI の 1 ページ目が canonical `device_id` を primary に表示し、silent install では別配布の `device-id-probe` が同じ値を表示する
- `device-id-probe` は通常は人間向け表示を行い、`--json` で machine-readable 出力も返せるようにする
- `device_id` は preregistration 完了前に採取し、管理者がサーバーへ事前登録しておく
- インストーラーは後追い登録フローを持たず、prereg-check で ready になったことだけを確認する
- Windows Service は通常運用で `device_id` を受け取らず、`expected_device_id` を保持したうえで runtime では `EK public` から毎回再導出した値だけを送信に使う
- runtime の再導出値が `expected_device_id` と一致しない場合、Service は blocked / mismatch state に入って enrollment / renewal を停止する
- `DEVICE_ID_OVERRIDE` は release では禁止し、通常運用には含めない
- `EK public` から canonical `device_id` を導出できない環境は、通常運用の Windows MSI 管理対象としては受け付けず、明示的な検証フロー以外では install を進めない
- cutover 以後に non-canonical `device_id` や `DEVICE_ID_OVERRIDE` ベースの値を使う既存 Windows 管理対象 client は reject し、canonical 値での再登録を必須にする
...
- `managed_client_type=windows-msi` の nonce issuance では、request に含まれる `ek_public_b64` から canonical `device_id` を再計算し、registered `device_id` と request `device_id` の両方に一致しない限り nonce を返さない
...
サーバー側 client state 方針:

- `managed_client_type=windows-msi` の通常運用で使う state は `INACTIVE`、`ISSUABLE`、`ISSUED` の 3 つに限る
- preregistration 直後の client state は `INACTIVE`
- 管理者が preregistration 完了後に初回発行用 secret を発行したときだけ `INACTIVE -> ISSUABLE`
- 初回発行が成功したら `ISSUABLE -> ISSUED`
- 通常の Windows MSI 自動更新は `ISSUED` のまま既存クライアント証明書 + TPM attestation で実施し、新しい enrollment secret は要求しない
- 証明書 expiry 後は `ISSUED -> INACTIVE` へ戻したうえで、管理者が新しい初回用 secret を発行して `INACTIVE -> ISSUABLE` に戻し、再 enrollment する
- TPM / vTPM 交換や VM 再作成で canonical `device_id` が変わった場合は、管理者が既存 client record の `device_id` を更新し、旧証明書を revoke して `INACTIVE` に戻したうえで再 enrollment する
- `UPDATABLE` / `PENDING` は server 全体の legacy/manual update path には残り得るが、`managed_client_type=windows-msi` では使わない
...
- 自動更新のたびに管理者が renewal secret を発行する運用は、Windows MSI の正常系に含めない

### 5. Preregistration Check Contract

- GUI MSI と silent install の `device-id-probe` は、`client_uid` + canonical `device_id` を受けて match / readiness だけを返す read-only prereg-check endpoint を使う
- prereg-check endpoint は unauthenticated で公開するが、rate limit を掛け、response は `client_not_found` / `device_id_mismatch` / `not_issuable_yet` のような限定的 reason code に絞る
- prereg-check endpoint は登録済み `device_id` や内部属性値そのものを返さない
- `CLIENT_UID` は高 entropy の opaque ID を前提にし、人間向け alias は別の管理対象として扱う

### 6. Attestation Trust Model

- `managed_client_type=windows-msi` では `activation proof` を常に必須にする
- `EK certificate` は取得できれば保持するが、検証の必須条件にはしない
- TPM identity の binding は final verifier だけでなく nonce issuance と activation path でも強制する
...
- cutover 後の non-canonical Windows 管理対象クライアントは reject し、canonical 値での再登録を前提にする
- `device_id` の正規化ルールを、`EK public` `SPKI DER` -> `SHA-256` -> lowercase hex として定義する
- Windows MSI の通常運用では `ISSUED` state の既存証明書ベース renewal を正式フローにし、`managed_client_type=windows-msi` では `UPDATABLE` / `PENDING` を reject する
...
- Windows MSI 管理対象 client の通常 renewal では、管理者の renewal secret 発行なしに `ISSUED -> renewal -> ISSUED` が成立する前提を文書化できる
...
- 設定ロード元を MSI プロパティと設定ファイルに整理し、`expected_device_id` を保持する
...
- runtime では `EK public` から `device_id` を毎回再導出し、`expected_device_id` mismatch を blocked state に落とす
- GUI MSI の 1 ページ目と、別配布の `device-id-probe` による preregistration 採取フローを支える
...
- 自動更新の正常系では、管理者が renewal secret を都度発行しなくても動くことを前提にする
...
- Windows MSI 管理対象 client の `INACTIVE -> ISSUABLE -> ISSUED` と `ISSUED -> renewal -> ISSUED` を正式 state model として明文化する
...
- 設定モデルを `server_url`, `client_uid`, `enrollment_secret`, `expected_device_id`, `renew_before`, `poll_interval` に整理
- GUI MSI の 1 ページ目と別配布の `device-id-probe` が canonical `device_id` を表示できるようにする
...
- GUI MSI を primary preregistration flow とし、1 ページ目で canonical `device_id` を表示する
- silent install では別配布の `device-id-probe` を prerequisite にする
- `device-id-probe` は通常は human-readable output を行い、`--json` で machine-readable output も返す
- `client_uid` は高 entropy の opaque ID を使い、人間向け alias は別管理にする
- `device_id` は事前登録前提にし、インストーラーは後追い登録フローを持たない
- Windows MSI の canonical `device_id` は normalized `EK public` `SPKI DER` の `SHA-256` を lowercase hex 化した値とする
- canonical `device_id` は GUI MSI の 1 ページ目または `device-id-probe` で採取し、管理者が preregistration する
...
- nonce 取得は専用 REST API を追加して実現する
- nonce issuance は `managed_client_type=windows-msi` では `ek_public_b64` から canonical `device_id` を再計算し、registered / request / TPM material が一致したときだけ許可する
- 初回発行用 secret は preregistration 完了後にだけ発行し、`INACTIVE -> ISSUABLE` に進める
- GUI MSI と silent install はどちらも prereg-check を fail-fast で通す。GUI では `not_issuable_yet` に対して同じ画面で `Retry` できる
- prereg-check は `client_uid` + canonical `device_id` を受ける read-only 専用 endpoint とし、unauthenticated で公開するが rate limit を掛け、response は限定的 reason code のみにする
- Windows Service は runtime で `device_id` を毎回 `EK public` から再導出し、`expected_device_id` を保持して mismatch 時は blocked state へ入る
- release 向け MSI / service / server では `DEVICE_ID_OVERRIDE` を完全に禁止し、test harness / debug build にだけ残す
- 初回発行成功後、enrollment secret は Windows 側から削除する
- 自動更新の認可は既存クライアント証明書ベースを第一候補にする
- Windows MSI の通常自動更新は `ISSUED -> renewal -> ISSUED` を正式フローとし、管理者の renewal secret 発行は要求しない
- `managed_client_type=windows-msi` の通常運用で使う state は `INACTIVE` / `ISSUABLE` / `ISSUED` に限り、`UPDATABLE` / `PENDING` の update-secret flow は禁止する
- 証明書 expiry 後は管理者が新しい初回用 secret を発行して `INACTIVE -> ISSUABLE` に戻し、再 enrollment する
- TPM / vTPM 交換時は管理者が既存 client record の `device_id` を更新し、旧証明書を revoke して `INACTIVE` に戻したうえで再 enrollment する
- `managed_client_type=windows-msi` では `activation proof` を常に必須にし、`EK certificate` は取得できれば保持するが検証の必須条件にはしない
- フェーズ 2 の TPM 検証必須ラインは署名検証、nonce 一致、CSR 鍵一致とする
- cutover 後に non-canonical `device_id` や `DEVICE_ID_OVERRIDE` ベースの Windows 管理対象 record は一律 reject し、再登録を必須にする
...
## Reassessment on 2026-03-27

今回の実装方針を source code と照合して再確認した結果、policy decision と implementation gap を次のように固定する。

### Policy Decisions Closed In This Review

- nonce issuance の時点から `ek_public_b64` ベースの canonical `device_id` binding を必須にする
- preregistration 採取フローは GUI MSI の 1 ページ目を primary にし、silent install 用には別配布の `device-id-probe` を用意する
- Windows Service は runtime で `device_id` を毎回再導出し、`expected_device_id` mismatch では blocked state に入る
- expiry 後 recovery は「新しい初回用 secret を発行して `INACTIVE -> ISSUABLE` に戻し、再 enrollment する」方式に固定する
- TPM / vTPM replacement は既存 client record の `device_id` を管理者が明示更新し、旧証明書 revoke + `INACTIVE` reset 後に再 enrollment する
- GUI MSI と silent install はどちらも prereg-check を fail-fast で要求し、GUI では `Retry` を提供する
- 初回用 secret は preregistration 完了後にだけ発行する
- `managed_client_type=windows-msi` では `UPDATABLE` / `PENDING` を完全に禁止し、初回 secret・証明書ベース renewal・expiry 後の再 enrollment だけを許可する
- cutover 後の non-canonical record は互換運用せず、一律 reject + 再登録にする
- `DEVICE_ID_OVERRIDE` は release では禁止し、test harness / debug build にだけ残す
- `activation proof` は常に必須にし、`EK certificate` は optional metadata として扱う
- prereg-check endpoint は unauthenticated + rate-limited + minimal response とし、`client_uid` は opaque ID を前提にする

### Newly Confirmed Considerations

- server は `managed_client_type=windows-msi` に対して `ek_public_b64` から canonical `device_id` を再導出し、nonce endpoint / activation start endpoint / final attestation verifier の各段で登録値と結び付ける実装へ更新済みである
- Rust service は runtime で現在の TPM identity を probe し、設定上の `expected_device_id` と不一致なら `BlockedDeviceIdentityMismatch` へ遷移して enrollment / renewal を止める実装へ更新済みである
- GUI MSI の 1 ページ目と helper-driven silent install はどちらも canonical TPM identity を前提にした prereg-check 導線を source 上に持つが、live GCP での再検証はまだ残っている
- 別配布の `device-id-probe`、opaque `client_uid` を前提にした preregistration UX、release build での `DEVICE_ID_OVERRIDE` 禁止は source と runbook の両方へ反映済みだが、cutover 後の運用 validation は継続して必要である
- server batch は証明書 expiry 時に `ISSUED -> INACTIVE` へ戻す。今回の合意により、この後続動作は「新しい初回用 secret を発行して再 enrollment」へ固定された
- `device_id` を managed directory 名や key name hint に使っている箇所があるため、cutover で non-canonical record を reject するときは、Windows 側の managed path / reinstall behavior / validation script も canonical-only 前提へそろえる必要がある

### Implemented Against The Revised Policy

- challenge-based initial issuance と signer-certificate-based renewal はすでに分離されており、renewal without secret は `cmd/scepclient` と server middleware の両方で成立している
- server は `managed_client_type=windows-msi`、`device_id` attribute、nonce endpoint、activation endpoint、quote verification、CSR public key binding を source に実装済みである
- helper は canonical attestation に `ek_public_b64` を載せられ、GCP vTPM 上でも `EK public` 自体は取得できる
- current GCP validation では `ISSUED` state の managed Windows client に対する same-key renewal が、new admin secret なしに成功している
- documentation 上の target policy として、通常 renewal を `ISSUED -> renewal -> ISSUED`、cutover 後の canonical-only `device_id`、credential-activation-first trust model は確定済みである

### Not Yet Implemented Against The Revised Policy

- GUI MSI の新しい `page 1 probe -> prereg-check -> ENROLLMENT_SECRET` 導線を live GCP 上で再検証すること
- helper-driven silent install fail-fast (`device-id-probe.exe -json` + prereg-check) を live GCP 上で再検証すること
- legacy fallback path は削除済みのため、packaging / prereg-check / runbook は WiX v4 GUI MSI を唯一の前提として揃えること
- cutover 後の non-canonical record reject と、expiry recovery / TPM replacement reprovision の runbook / validation を追加で詰めること

### Recommended Implementation Order

1. **Revalidate end-to-end**
   - GUI MSI の `page 1 probe -> prereg-check -> ENROLLMENT_SECRET` を GCP で再検証する
   - silent install helper の fail-fast prereg-check と same-key renewal を GCP で再検証する
2. **Close cutover and recovery gaps**
   - non-canonical record reject の cutover plan を canonical-only runbook へ落とす
   - expiry recovery と TPM replacement reprovision を validation script / runbook に反映する

## Current Implementation Status

最終更新日: 2026-04-02

この節は、直近の source code と GCP 検証環境の実測結果をまとめた引き継ぎ用スナップショットである。今回の更新では、2026-03-30 までの fresh reset validation に加え、Linux-host の `build_windows_msi.sh` 復旧、WiX v4-authored MSI による fresh issuance (`copilot-install-20260401T083450Z-7118`)、および tampered activation renewal negative rerun (`copilot-install-20260402T020037Z-12035`) を統合している。本更新時点では検証環境そのものは引き続き Terraform 管理下にあり、`scep-server-vm` と `scep-client-vm` は **RUNNING**、current IP は server=`10.42.0.5`、client=`10.42.0.6` であり、どちらも external IP を持たない internal-only topology で運用している。
...
- live `scep-server-vm` には current local `scepserver-opt` を再配備済みで、canonical `attestation_e2e_canonical.sh` と `scep-client-vm` からの real canonical `PKIOperation` の両方について journal 上で複数回 `error=null` を確認した
...
- Linux-host WiX v4 verification は current devcontainer で復旧済みであり、`build_windows_msi.sh` は `build/windows-msi/installer/dist/MyTunnelApp.msi` を生成し、live GCP positive / negative validation の両方でその artifact を current helper flow に載せられている
...
- server verifier は canonical `tpm2-windows-v1` のみを受け付け、legacy / placeholder attestation format は reject する
...
- `build_windows_msi.sh` の既定 stage dir は `build/windows-msi` へ移行済みで、generated installer 入力を source tree の installer 定義と分離済みである。packaging backend は WiX v4 (`installer/main.wxs` + `WixToolset.UI.wixext`) のみに統一済み
...
- `infra/terraform/scripts/test/windows_canonical_renewal_e2e.sh` と `windows_activation_negative_renewal_e2e.sh` は current WiX v4-authored bootstrap を raw に検証できる。`LocalService` runtime validation は必要に応じて lower-level `install_windows_msi.sh --converge-to-local-service` で別途実施する
- `installer/main.wxs` では registry-searched existing values と public input properties を分離し、default / existing / explicit input の precedence を custom action で表現する slice を追加した
...
- Linux-host `build_windows_msi.sh` は current devcontainer で再び通る状態に戻っており、`installer/main.wxs` の prior WiX v4 authoring errors (`WIX0005`, `WIX0400`) は解消済みである。current focus は authoring bring-up ではなく、validated bootstrap account と runbook の整合を詰めることに移っている
...
- supported path は WiX v4-authored MSI であり、runtime convergence helper (`install_windows_msi.sh --converge-to-local-service`) を通した live GCP validation では fresh install issuance と same-key renewal の両方が `NT AUTHORITY\\LocalService` で成立した
...
- Linux-host の `build_windows_msi.sh` は current devcontainer で成功しており、live GCP positive / negative validation でもその build output を current helper flow に載せられている。Phase 1 側の残課題は、same-key renewal を exact current `LocalSystem`-authored WiX path で取り直すかどうかと、service-account policy を明文化することに寄った
- supported helper path (`install_windows_msi.sh --converge-to-local-service`) では live GCP で `LocalService` runtime へ収束できる
...
- latest Linux-host WiX verification では `build_windows_msi.sh` が current devcontainer 上で成功し、generated MSI を live GCP positive / negative validation にそのまま利用できている。old `WIX0005` / `WIX0400` blocker は解消済みである
...
- `attestation_e2e_canonical.sh` は canonical `tpm2-windows-v1` helper であり、synthetic OpenSSL-generated quote/signature を使って server semantics を検証する
...
- historical note: 2026-03-25 の fallback authoring investigation では、silent MSI semantics を `LocalService + uninstall cleanup` へ完全には寄せられないことを確認した。current source は WiX v4-only であり、supported build path は `build/windows-msi` staging から `wix build -arch x64 -ext WixToolset.UI.wixext -o ... installer/main.wxs` を呼ぶ
