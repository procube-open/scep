...
fn probe_windows_machine_store(
    ...
    let script = build_windows_machine_store_probe_script(
        &plan.server_url,
        &plan.client_uid,
        &windows_paths::key_dir_name(&plan.client_uid, &plan.expected_device_id),
        &managed_cert_path,
    );
...
$expectedKeyName = '{expected_key_name}'
...
function Get-MyTunnelCertificateKeyName {{
...
if (-not [string]::IsNullOrWhiteSpace($serverActiveThumbprint)) {{
  $cert = Get-ChildItem Cert:\LocalMachine\My |
    Where-Object {{
      $_.Thumbprint -eq $serverActiveThumbprint -and
      (Get-MyTunnelCertificateKeyName -Cert $_) -eq $expectedKeyName
    }} |
...
if ($null -eq $cert -and $null -ne $managedCertThumbprint) {{
  $cert = Get-ChildItem Cert:\LocalMachine\My |
    Where-Object {{
      $_.Thumbprint -eq $managedCertThumbprint -and
      (Get-MyTunnelCertificateKeyName -Cert $_) -eq $expectedKeyName
    }} |
    Sort-Object NotAfter -Descending |
    Select-Object -First 1
...
if ($null -eq $cert) {{
  $cert = Get-ChildItem Cert:\LocalMachine\My |
    Where-Object {{
      $_.GetNameInfo([System.Security.Cryptography.X509Certificates.X509NameType]::SimpleName, $false) -eq '{client_uid}' -and
      (Get-MyTunnelCertificateKeyName -Cert $_) -eq $expectedKeyName
    }} |
    Sort-Object NotAfter -Descending |
    Select-Object -First 1
...
"#,
        ...
        expected_key_name = escape_ps_single_quoted(expected_key_name),
...
mod tests {
    ...
    fn machine_store_probe_script_prefers_server_active_certificate() {
        ...
        assert!(script.contains("$expectedKeyName = 'client-001-device-001'"));
        ...
        assert!(script.contains("Get-MyTunnelCertificateKeyName"));
        ...
        assert!(script.contains("$_.Thumbprint -eq $serverActiveThumbprint -and"));
        assert!(script.contains("$_.Thumbprint -eq $managedCertThumbprint -and"));
        assert!(script.contains("X509NameType]::SimpleName"));
        assert!(script.contains("-eq 'client-001'"));
        assert!(script.contains("-eq $expectedKeyName"));
