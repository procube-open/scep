...
2. 少なくとも以下を更新します。
   ...
   - internal-only 運用にする場合、`scep_source_ranges` は内部 CIDR のみでよく、VM へ外部 IP を付ける必要はありません
...
- `server_external_ip`, `client_external_ip`（internal-only 構成では空文字）
...
internal-only 構成では public RDP は前提にしません。GUI 操作が必要な場合は Chrome Remote Desktop を優先し、同一 VPC / VPN / IAP 等で内部到達できる場合のみ `CLIENT_IP` へ RDP 接続してください。
