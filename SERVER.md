...
uid は文字列で必須です。attributes はオブジェクト型であれば任意に設定でき、設定されていない場合は`{}`として登録されます。

現在のサーバでは `attributes` に以下の値を設定できます。

- `device_id`
  - 文字列。
  - 前後空白を除去し、小文字化して保存されます。
  - 設定されている場合、SCEP の attestation に含まれる `device_id` と一致しない要求は拒否されます。
- `attestation_aik_spki_sha256`
  - 文字列。
  - `AIK` の SubjectPublicKeyInfo DER 全体の SHA-256 フィンガープリントです。
  - `aa:bb:...` や `aa-bb-...` 形式でも受け付けますが、保存時は 64 文字の小文字 16 進文字列に正規化されます。
  - 設定されている場合、attestation に含まれる `aik_public_b64` の SHA-256 と一致しない要求は拒否されます。
- `attestation_ek_cert_sha256`
  - 文字列。
  - `EK certificate` DER 全体の SHA-256 フィンガープリントです。
  - `aa:bb:...` や `aa-bb-...` 形式でも受け付けますが、保存時は 64 文字の小文字 16 進文字列に正規化されます。
  - 設定されている場合、attestation に含まれる `ek_cert_b64` の SHA-256 と一致しない要求は拒否されます。
- `managed_client_type`
  - 文字列。
  - 現在の正式値は `windows-msi` のみです。
  - `windows-msi` の場合、その client は Windows MSI 管理対象として扱われ、`device_id` が必須になります。
  - canonical `tpm2-windows-v1` attestation では `activation_id` と `activation_proof_b64` が必須になり、server-issued credential activation challenge と一致しない要求は拒否されます。
  - 未設定 client でも payload に activation proof が含まれていれば検証されます。

`device_id` は空文字列を許容しません。`attestation_aik_spki_sha256` と `attestation_ek_cert_sha256` は 64 文字の SHA-256 フィンガープリントである必要があります。`managed_client_type` は `windows-msi` に正規化されます。deprecated な `attestation_activation_required` は add/update API では受け付けません。

### Attestation nonce 取得(POST `/api/attestation/nonce`)

credential activation / quote attestation の前処理として、client ごとの one-time nonce を払い出します。
...
`client_uid` / `device_id` が登録済み client attribute と一致しない場合、nonce は払い出されません。

### Attestation activation challenge 取得(POST `/api/attestation/activation/start`)

credential activation を使う client は、この endpoint に `AK AttestationParameters` と `EK public` を送り、server-generated challenge を受け取ります。
...
- `aik_tpm_public_b64`
- `aik_create_data_b64`
- `aik_create_attestation_b64`
- `aik_create_signature_b64`
- `ek_public_b64`
- `use_tcsd_activation_format` (optional)
...
server は `device_id` と未消費 nonce を照合したうえで challenge を発行します。最終的な `PKIOperation` では attestation payload に `activation_id` / `activation_proof_b64` を含め、server 側で one-time verify されます。
...
`attributes` のバリデーションと正規化ルールは [クライアント追加](#クライアント追加post-adminapiclientadd) と同じです。
